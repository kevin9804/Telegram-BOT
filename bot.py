...
@bot.message_handler(commands=['fido_home_internet'])
def send_fido(message):
    photo = open('info/fido/internet/Internet.jpg','rb')
    bot.send_photo(message.chat.id, photo, caption='图中为FIDO家庭网络套餐\nThere are fido home internet plan in the pictures')

@bot.message_handler(func=lambda msg: 'ROGERS' or 'BELL' in msg.text)
def send_fido(message):
    bot.send_message(message.chat.id, '很抱歉，此部分资料暂时不能与您分享\nUnfortunately, I am not avaialbe to share this plan with you right now.')
